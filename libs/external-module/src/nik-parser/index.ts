import { kabkot, kecamatan, provinsi } from './wilayah.json';
...
export const nikParser = (nik: string): NikDetail => ({
  ...
  kelamin(): string {
    return this.lahir().getDate() < 40 ? 'LAKI-LAKI' : 'PEREMPUAN';
  },

  lahir: (): Date => {
    let year = Number(nik.substring(10, 12));
    const month = Number(nik.substring(8, 10));
    const date = Number(nik.substring(6, 8));

    if (year < Number(new Date().getFullYear().toString().substr(-2))) {
      year = year + 2000;
    }
